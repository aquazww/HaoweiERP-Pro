    ...
    <div class="page-content">
      <div class="toolbar-card">
        <div class="toolbar-left">
          ...
          <el-select v-model="statusFilter" placeholder="状态筛选" clearable style="width: 140px" @change="loadSuppliers">
...
const validateCode = (rule, value, callback) => {
  if (!value || !value.trim()) {
    ...
    callback(new Error('供应商编码不能超过50个字符'))
  } else if (!/^[A-Za-z0-9_-]+$/.test(value)) {
    callback(new Error('供应商编码只能包含字母、数字、下划线和横线'))
  } else {
    callback()
  }
}
...
const loadSuppliers = async () => {
  ...
  try {
    ...
    suppliersList.value = res.data.items || res.data.results || []
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateSupplier(form.id, submitData)
...
.suppliers-page .data-table :deep(.el-table__cell) {
  padding: 10px 0;
}

.suppliers-page .data-table :deep(.el-table__body-wrapper) {
  overflow-x: auto;
}
