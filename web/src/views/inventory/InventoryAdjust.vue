  ...
  <div class="adjust-page">
    <div class="page-content">
      ...
      <div class="table-card">
        <el-table :data="adjustList" style="width: 100%" v-loading="loading" :height="tableHeight" class="data-table" stripe>
          <el-table-column type="index" label="#" width="50" align="center" />
          <el-table-column prop="order_no" label="调整单号" width="160">
            <template #default="{ row }">
              <span class="order-no-badge">{{ row.order_no }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="warehouse_name" label="仓库" width="120" />
          <el-table-column prop="adjust_type_display" label="调整类型" width="100">
            <template #default="{ row }">
          ...
          <el-table-column prop="reason_display" label="调整原因" width="100" />
          <el-table-column label="调整明细" min-width="200">
            <template #default="{ row }">
    ...
    <!-- 查看弹窗 -->
    <el-dialog v-model="viewDialogVisible" title="调整单详情" width="800px" class="view-dialog" destroy-on-close>
      <el-descriptions :column="2" border class="view-descriptions">
        <el-descriptions-item label="调整单号">{{ viewData.order_no }}</el-descriptions-item>
        <el-descriptions-item label="仓库">{{ viewData.warehouse_name }}</el-descriptions-item>
        <el-descriptions-item label="调整类型">{{ viewData.adjust_type_display }}</el-descriptions-item>
        <el-descriptions-item label="调整原因">{{ viewData.reason_display }}</el-descriptions-item>
      ...
      <el-table :data="viewData.items" border style="width: 100%;">
        ...
        <el-table-column prop="before_quantity" label="调整前" width="100" align="right" />
        <el-table-column prop="adjust_quantity" label="调整数量" width="100" align="right">
          <template #default="{ row }">
        ...
        <el-table-column prop="after_quantity" label="调整后" width="100" align="right" />
      </el-table>
    </el-dialog>
  </div>
</template>
