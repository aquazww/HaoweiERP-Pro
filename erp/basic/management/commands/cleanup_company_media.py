    ...
    def handle(self, *args, **options):
        ...
        if company:
            if company.logo_filename:
                used_files.add(company.logo_filename)
                self.stdout.write(f'使用中的Logo: {company.logo_filename}')
            if company.stamp_filename:
                used_files.add(company.stamp_filename)
                self.stdout.write(f'使用中的印章: {company.stamp_filename}')
